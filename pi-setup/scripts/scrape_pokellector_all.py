...
Drop into the PokeScraper_3.0 folder and run:
    python scrape_pokellector_all.py            # all species
...
Auto-recycles chromedriver on read-timeout / dead-session errors.
"""
...
DEAD_DRIVER_HINTS = (
    "Read timed out", "HTTPConnectionPool", "invalid session id",
    "no such window", "disconnected", "session deleted",
    "chrome not reachable", "Max retries exceeded",
)
...
def is_dead_driver(exc: Exception) -> bool:
    s = str(exc)
    return any(h in s for h in DEAD_DRIVER_HINTS)


def recycle(processor, which):
    log(f"  [recycle] restarting {which} chromedriver")
    if which == "scraper":
...
def scrape_one(processor, name):
    t0 = time.time(); cards = []
    for source, fn in (
        ("eng", lambda: processor.scraper.fetch_card_data(
            ps.Config.ENG_URL,
            processor.scraper.fetch_page_urls(ps.Config.ENG_URL, name),
            "Pokellector English", processor.set_loader)),
        ("jp", lambda: processor.scraper.fetch_card_data(
            ps.Config.JP_URL,
            processor.scraper.fetch_page_urls(ps.Config.JP_URL, name),
            "Pokellector Japanese", processor.set_loader)),
        ("pc", lambda: processor.pricecharting_scraper.fetch_price_data(
        ...
        try:
            ...
            log(f"  [{source}] {name}: {e}")
            if is_dead_driver(e):
                recycle(processor, "pricecharting" if source == "pc" else "scraper")
    ...
    try:
        for i, name in enumerate(names, start=1):
            ...
            try:
                ...
                recycle(processor, "scraper")
                recycle(processor, "pricecharting")
    finally:
        try: processor.scraper.close()
        except Exception: pass
        try: processor.pricecharting_scraper.close()
