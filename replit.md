...
*   **Pi 5 V3D + wayland-native chromium = GPU process crash**: on labwc 0.9.2 + Bookworm chromium (apt), the GPU process dies within a few seconds (`GPU.GPUProcessTerminationStatus2 = 4`), taking the browser with it. Diagnosed via `--enable-logging=stderr --v=1`. The reliable workaround for kiosk use is **software rendering**: `--disable-gpu --use-gl=swiftshader` in the *default* flag set (not just fallback). Two small kiosk pages render fine on Pi 5 CPU. Wayland-native is still required because XWayland + labwc's `MoveToOutput HDMI-A-2` is broken.
*   **Chromium needs `--no-sandbox` when launched outside a logind session**: the kiosk service starts labwc via PAM but chromium's seccomp/userns sandbox can still fail silently (no stderr) when there's no controlling tty session. Always pass `--no-sandbox` for the kiosk launcher; the user-data-dirs are already isolated to `/home/ngansen/.hanryx/{admin,kiosk}-profile`.
