...
*   **naver shopping IP-bans the trade-show egress**: `https://search.shopping.naver.com/...` returns a styled HTML page titled `네이버쇼핑` whose body reads "쇼핑 서비스 접속이 일시적으로 제한되었습니다" (shopping service access temporarily restricted). Their reason list explicitly includes "VPN을 사용하여 접속한 IP" — once the egress IP is flagged, no amount of UA spoofing, header tuning, Playwright, or `playwright-stealth` defeats it (the block is at the network layer, applied BEFORE TLS fingerprinting matters). The page is small (~3.2KB) and contains zero product anchors, so detection is easy: any naver fetch <5KB with `content_error` class = blocked. Only fixes today: (a) route naver traffic through a residential proxy, (b) accept that naver pricing chips will be empty on this Pi. NOT a code bug — `naver()` in `price_scrapers.py` correctly returns `[]` and the rest of the response renders without it.
